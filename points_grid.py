...
class cloth:
    def __init__(self, Points, interpolation="TIN", cellsize=5, displacement=1, mass=1):
        ...
        self.bounds = [min(Points[:, 0]), max(Points[:, 0]), min(Points[:, 1]),
                       max(Points[:, 1])]  # define bounding box
...
class QueryTree(
    KDTree):  # useless, will cause error, it aims to find the near points that exert internal force to each other
...
if __name__ == "__main__":
    ...
    clothes.draw_particles(show_OriginalZ=True, zmin=-clothes.MinZ,
                           zmax=-clothes.MaxZ)  # check original points before ground filtering
